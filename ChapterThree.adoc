...
The first way to use monads with ALA is to use the monad interface, such as IObservable, for the ports on domain abstractions. For example, we could have a domain abstraction for a UI grid, and another domain abstraction for a CSV file reader. The grid has a IObservable input port and the CSVFileReader has an output IObservabe port. We may want to do some filtering and selecting inbetween, and use monads that alreay exist in the reactive extension library for that.
