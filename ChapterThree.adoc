...
ALA and monads are both composition patterns using layers. The main difference is that ALA composes instances of classes whereas monads are for composing functions.

ALA is a three layer pattern. Monads are of themselves a two layer pattern, corresponding with the application and programming paradigm layers of ALA. Expressions that compose functions using Bind are in the top layer. The Monad Type or Interface<T> and the Bind function are in the programming paradigms layer. They are highly reusable and are often provided as a library.

In the monad world, it is common to add many functions that do more specific jobs than the basic Bind function, such as Filter, Map, Select and Aggregate. These functions use Bind, or do the equivalent of Bind themselves. This set of functions are roughly equivalent to the domain abstractions layer. So it could be said that monads are an example of the ALA layering pattern.  

ALA is generally easier to understand and more versatile than monads.

ALA is easier to understand because it uses plain objects and therefore doesn't hide so much under the covers in the form of closures. Monads usually involve a number of closure objects and delegate objects wired together in a large structure.

In ALA, what would be closures for monads, become pain objects which you instantiate yourself with WireIn(new Something()). The only new thing you have to learn about ALA objects is that they must use ports for their I/O. You are not allowed make a class directly have an association with another class. You must make a port. That is just dependency injection. The only difference between ALA dependency injection and more conventional dependency injection is that ALA wiring must be explicitly specified in a cohesive user story abstraction in the application layer. And teh WireTo function uses reflection to implement dependency injection instead of constructor injection or setter injection. That's pretty much it to understanding the mechanics of ALA over conventional objects.  

ALA is more versatile, again because it uses plain objects with ports. The monad Bind function, because it is a function, essentially has only two ports, an input port and an output port. Both ports are of the same type, e.g. IEnumerable. Some more specific functions can have a second input port, e.g. Merge will merge two streams. ALA domain abstractions can have an arbitrary number of ports, and they can be using arbitrarily different programming paradigms.
...
So ALA domain abstractions are a bit like Integrated circuits in electronics (many pins, with different functions), and monads are more like resistors and capacitors (two pins, analog only). 

For the particular problems that monads suit, the monad syntax is more concise.
...
If monads are already available in a system, you can have the best of both worlds by using them both together. For example, if you have a library of IEnumerable, or IObservable based monads already, it wouldn't make sense to rewrite every function in teh library such as Select as wirable domain abstractions. Instead, what you can do is create a single domain abstraction that can be configured with monad code. For example, we can write a domain abstraction called EnumerableQuery that can be configured with any LINQ IEnumerable query:
...
EnumerableProxySource is a small class inside the EnumerableQuery abstraction that acts as an IEnumerable on which we can start a LINQ query. When EnumerableProxySource is passed into EnumerableQuery, the EnumerableQuery configures it with a source Enumerator via a setter. So when the EnumerableProxySource's GetEnumerator is called later, it will return an actual Enumerator.

We create a LINQ query once and use it three times (which you wouldn't normally do, I'm just using the same one three times to save having more code for different ones). I wanted to demonstrate wiring three EnumerableQuerys together just for fun.

The second half of the code is conventional ALA user story code that instantiates three instances of EnumerableQuery. The input of the first one is wired from a very simple source consisting of an IEnumerable that returns one value which is a zero. The last one is wired to a EnumerableToConsole domain abstraction, which does was you might expect - when told to run, it loops through the IEnumerable at its input and outputs all the values to the console. Here is the output of the program:
...
One unusual thing you may notice about this ALA program is the use of WireInR instead of WireIn that we would normally use to chain wire things. A.WireInR(new B()) actually wires from B to A. You use it like you would WireIn, in the same direction as the dataflow, but it actually wires in the opposite direction. This is because the IEnumerable, the programming paradigm interface being used, goes in the opposite direction as the dataflow. The B object has a filed of type IEnumerable and the A object implements IEnumerable. So the wiring must go in teh reverse direction of the dataflow. WireInR is implemented simple as WireInR(A,B) {WireIn(B,A);}  

Further discussion about the domain abstraction EnumerableQuery and a full comparison with monad can be found in chapter six.
