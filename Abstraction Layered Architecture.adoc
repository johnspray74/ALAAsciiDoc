...
ALA gives us two fundamental architectural constraints when writing software:

--
- The only relationship allowed is a dependency on an abstraction. That abstraction must be significantly more abstract than the abstraction containing the dependency. No other relationships are needed or allowed.

- All abstractions must be small - 100-500 LOC (or diagram nodes).
--


. Chapter 1, "What problem does ALA solve". These two constraints solve the "big ball of mud" problem, and other related software engineering problems by providing a pre-worked meta-architecture or macro-structure that works for all software.

. Chapter 2, "What does the structure look like?". Without explaining how or why, describes what the emergent structure looks like after applying these two constraints. It describes: What an abstraction is in terms of (usually) one class or function or a small group of highly cohesive code artefacts. The most common pattern that emerges, which is to explicitly wire together instances of abstractions that communicate with each other through 'abstract interfaces'. The emergent use of abstraction layers and the standard folders for these layers. The emergent use of ports and how to make ports using plain code inside classes. The emergent use of diagrams. The use of programming paradigms when composing instances of abstractions. That multiple programming paradigms are used in an application. e.g. user interface layout, events, data flows, activity flows, state machines, data schemas, and others. The emergent property of the direct representation of requirements in a formal way, like a DSL, which is then executable.

. Chapter 3, "Why the structure works". Explains the theory behind these two fundamental architectural constraints and that they actually result in zero coupling between all abstractions. (Not just loose coupling, zero coupling.) In turn, this eliminates complexity. Explains that requirements typically inherently contain a graph structure, and that this graph should be explicit, preferably in a diagram.

. Chapter 4, "Execution models". Explains how the underlying execution for each programming paradigm can be made to actually work through 'abstract interfaces'. Explains the ways that the CPU executes the right code at the right time when the programming paradigms that emerge from ALA organise code in an orthogonal way to imperative execution flow.

. Chapter 5, "Methodology". Describes ALA in terms of where it fits into Agile software development, and the use of an "Iteration zero" to do an initial representative design.

. Chapter 6, "The philosophy behind ALA". Gives the theory of why ALA works from the perspective of complex systems and how our brains work. It describes how the two fundamental architectural constraints are designed to mimic how our brains see the real world. This leverages how our brains have evolved to use abstractions to understand the real world to enable it to understand our programs in the same way.

. Chapter 7, "ALA compared with...". Unsurprizingly, the two fundamental constraints of ALA emerge many of the same architectural styles and patterns we find in software engineering. But it usually modifies them. Such things as layers, dependency injection, observer pattern, DSLs, components and connectors. But it also leaves some significant ones out. For example, ALA says that UML class diagrams are evil. This chapter goes through all the common large-structure styles and patterns. Choose the one you are most familiar with to see how ALA encompasses it, usually in a modified way, or contrasts with it. 
