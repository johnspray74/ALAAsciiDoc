...
No other relationships are allowed (or needed). This implies that the only unit of code in ALA is an abstraction. The term 'module' is not used because modules tend to collaborate (are loosely coupled). The term "abstraction" is used instead because abstractions are zero-coupled with one another.

Because an abstraction that is depended on must be significantly more abstract, the abstractions form layers. We give them names: Application layer, Domain Abstractions layer and Programming Paradigms layer. A small number of other layers are sometimes used for large applications.

There is no 'hierarchical' structure - abstractions cannot be contained within abstractions. We use layers instead because lower layer abstractions are more abstract, and therefore more reusable, and need to be public.

These constraints may seem extreme at first, but it turns out to be surprisingly easy, and rewarding, to write code that conforms to them. 

Patterns and properties emerge from these two constraints. In many ways these patterns are similar to ones we already know, which is not surprising - such things as Domain Specific Languages, Dependency Injection, Composite and Decorator patterns etc. However, there are often subtle but important differences. For example, in the observer pattern (publish subscribe), the observer cannot subscribe itself - a higher layer abstraction must do the subscribing for it. 

An emergent property is "zero coupling and high cohesion". This is qualitatively different from the familiar "loose coupling and high cohesion". What is implicit collaboration between modules in conventional code becomes explicit cohesive code contained within a more specific abstraction in a higher layer in ALA.
...
*Chapter 5, "Methodology". Describes ALA in terms of how it fits into Agile software development*
...
Every chapter ends with an example project, and we do the same here. Unlike most pedagogical sized examples, these example projects are progressively non-trivial. Yet because of ALA's power, they remain uncomplicated and easy to understand. 
...
In conventional code, we would make two modules for the switch and light, but the switch might  directly call the light, or vice versa. In ALA you can't do that. The concepts of Switch and Light are domain abstractions already handed to us in the words of the requirements. We must keep them as abstractions - they cannot know about each other, or any applications they are used in.

The application, as stated by the requirement and represented by the diagram, is an abstraction in itself. It is clearly at a different abstraction level to either the switch or the light. Only it knows that our specific application is a switch and a light connected together. 

In ALA, we frequently use a *Wiring pattern*, which consists of instantiating domain abstractions and wiring them together using an even more abstract interface representing a programming paradigm: 
...
Now that we know the Wiring pattern, let's quickly demonstrate the ease of maintenance on our application:
...
We also just invented another programming paradigm. It is used between Main and its children, and could be made to work say by poling:
...
The astute reader will notice that the AndGate can't implement IDataFlow<bool> twice. In later projects, we will show how we solve this implementation problem, and allow more than one input port of the same type.

You may also notice that the fanout from the output of the AndGate to both the Light and the Indicator won't work. We show how this problem is solved in later projects also.
...
We just invented another programming paradigm for "UI layout". It is used between all the UI elements: MainWindow, Menu, Tickbox, StatusBar, Indicator.
...
Don't worry, we won't be creating new programming paradigms at this rate for long. In fact we already have most of the ones we will ever use in all our example projects.
