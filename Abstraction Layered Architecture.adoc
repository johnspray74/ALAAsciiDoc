...
[.float-group]
--
image::BOSS_Great_Wall.jpg[BOSS_Great_Wall.jpg,450, title="In software, we lose our view of the large scale structure", float="right"]

*Engineering in the large scale structure of code*
--
...
If you just want a quick summary of ALA read section 2.1 in Chapter 2.



*Organisation of this site:*

** Chapter One, "What problem does ALA solve" explains the current major crisis in software engineering.

** Chapter Two, "What does the structure look like?" without explaining how or why, decribes the structure.
...
** Chapter Seven, "ALA compared with...", compares ALA with existing architectural styles and patterns, both similarities and differences. Choose the one you are most familiar with to see how ALA uses it, builds on it, or contrast with it. 
...
image::Cynefin.jpg[Cynefin.jpg,500, title="Code can quickly get complex", float="left"]
...
=== ALA uses just one type of relationship
...
The relationship means that, to read and understand the code inside A, you must know the abstraction B - not how the insides of abstraction B work, just what B does. The word "abstraction" implies that it should be learnable in a short time.

An example: A is an abstraction that calculates standard deviation. B is an abstraction that calculates squareroot. The code in A "requires knowledge of the squareroot to understand".

This "requires knowledge of to understand" relationship applies at design-time. So you can never use a relationship for a run-time dependencies. From now on we will call this type of relationship a "knowledge dependency".
...
Architectures generally work by applying constraints. ALA's constraint is that you have this one type of relationship. This fundamental constraint derives all of ALA's consequences, properties and patterns. A few of these these are:

. When data or events need to move from one part of a program to another, you can't use a relationship to do it. Abstractions don't know where their data comes from, nor where it goes.

. Because the target of a knowledge dependency must be more abstract, abstractions arrange themselves into layers. This is what gives the architecture its name: Abstraction Layered Architecture. Because these dependencies have noting to do with run-time dependencies, these layers are very different from the ones you might find in a conventional program that uses a layering pattern. Abstraction basically means reusable. As you go down the layers, abstraction increases and so does reuse. 

. Because the target abstraction must be clearly more abstract, the abstraction layers from themselves into discrete levels. Only a small number of abstraction levels are needed. ALA generally uses four or five levels. The layers are given standard names that describe their level of abstraction, e.g. The Domain Abstractions layer.

. ALA has zero coupling between all abstractions. Unfortunately there is a meme in the software engineering industry that there must be some coupling between software artefacts if the system is to do anything. We hear of "loose coupling" as being the ideal. This is completely incorrect. ALA uses zero coupling. In our A & B example above, the code inside B knows nothing of A. The code inside A, while it knows about the abstraction B, knows nothing about the code inside B. So we already know how to do it. ALA just constrains us to do it all the time.
...
. The knowledge dependency relationship type differs from the UML class composition relationship, or any UML relationship for that matter. Although it is often implemented as a UML association (directed arrow), the meaning is far more constrained. It must be "requires knowledge of to understand". In the UML you can use associations for getting data at run-time, but in ALA you can't. 

. A conventional program will typically have many many dependencies. They are there because data, messages, events, execution flow, etc, need to get from one place to another in the program. None of these dependencies exist in an ALA program. The whole difficult concept of dependency management simply vanishes. So where do these relationships go? How can the program still work?  How is it possible for messages and events to travel around at run-time if there is absolute zero coupling? In later chapters we will go into detail on how this works, but for now, the short answer is that all routings become a line of code inside another abstraction. 

. In saying ALA has zero coupling, of course there may be leakage coupling. The quality of the abstractions is important. For example, the code inside abstraction A could potentially tell how long B takes to execute. If it is sensitive to that, this is not a problem with ALA, but with the leaky abstraction. The design needs to be changed to eliminate the dependency on that leakage, or, if that is not possible, the coupling managed. But for the vast majority of code, the coupling between the insides of any two abstractions really is zero. 

. An abstraction's implementation can consist of a group of artefacts: local variables, enums, delagate types, interfaces, methods, classes, etc. These artefacts have high cohesion with one another. They are all considered one abstraction. They interconnect with each other, unconstrained. There are no dependency rules among any of the artefacts inside an abstraction. They are all considered part of the same code. The only constraint ALA makes is that their total must be small - of the order of 100-500 lines of code. This applies to all abstractions, including the one that is the application itself in the top layer. Later we will discuss the forces that drive this optimal size.

. Abstractions are the units of code in ALA. They are the only type of encapsulation. Programming languages don't support any concept that corresponds with this idea of an abstraction. So we need to improvise to create the logical grouping, even it can't be an encapsulation enforced by the compiler. (Note that namespaces are not encapsulations). I usually use a source file to represent a single abstraction.

. If the code inside an abstraction becomes too large, factor out an abstraction. The factored out abstraction must be more abstract. It can know nothing of the purpose of the abstraction it came from, so it is not just a specific component. It goes in a lower layer, where it can be reused.

. ALA has no concept of hiercharchies. Abstractions cannot be contained by other abstractions. They are never nested. The reason they are never nested is simple. An abstraction being used must be more abstract than the abstraction using it. Therefore it must be outside the abstraction using it so it can be reused elsewhere. ALA uses abstraction layers instead of encapsulation hierchaies. In this respect abstractions differ from components in that a components tend to provide a specific part for something. In other words components can be contained by the component that uses it. More on this in later chapters.

. Despite the apparent simplicity of the "Standard deviation uses squareroot" example, the constraint of only being allowed to use knowledge dependencies is not trivial to do all of the time, especially in object oriented programming. But once the emergent programming paradigms and patterns are practised, it becomes relatively easy. More on them in later chapters.

. Perhaps there is one aspect of ALA that is hard to master - the invention of appropriate abstractions is relatively difficult. The reason why the "standard deviation uses squareroot" example seems easy is that the squareroot abstraction was already invented for us, and we know it so well. In ALA you will need to invent your own domain level abstractions. But all whom I have taught how to do this have found it worth the effort. Working in the resulting code becomes an joy. 

. The effect of change to any one part of the program, whether in higher layers or lower ones, has greatly reduced ripple effects when compared with conventionally written programs. A ripple generally stops at an abstraction. And since all relationships are just uses of abstractions, ripples always stay inside a single abstraction. Since abstractions are just concepts they are relatively stable. The quality of the abstractions is important however. The most common change that ripples out is to make an abstraction more abstract to increase it's reuse. 
...
[source,C]
 ...
 void main()
 {
    int temperatures[NREADINGS];
    ConfigureTemperaturesAdc();
    while (1)
    {
        GetTemperaturesFromAdc(temperatures); // gets 100 readings in dma mode
...
[source,C]
 ...
 float SmoothTemperature(temperature) 
 {
    static filtered = 0;
...
[source,C]
 ...
 {
    // configure ADC port 2 to do DMA NREADINGS values at a time
 }
...
At first this code wont look that bad, because the whole program is so small.

As we are taught to do, different aspects of the thermometer implementation have been separated out into smaller pieces that we can understand. However, all the pieces are in some way collaborating to make a thermometer. They are all, therefore, coupled in some way, either explicitly or implicitly. So, we have to read all the code to understand the thermometer. Scale this up to 5000 lines of code, and we will have a big mess.
...
[source,C]
 ...
 void OffsetAndScale(data, offset, scale) 
 {
...
[source,C] 
 ...
 float Filter(int input, int filterStrength)  
 {
...
.adc.c
[source,C]
 void Configure(int port, int dmaBatchSize)
 {
    // configure dma for port
 }
 float GetAdcReadings(int temperatures[], int port, int damBatchSize) 
 {
    for (i = 0; i<dmaBatchSize; i++) {
        temperature[i] = Port(port);
    }
 }
...
....
 void main()
 {
    program = new ADC(port=2,dma=100)
    .foreach()
...
==== Composing using multiple programming paradgms:
...
Now we are going to build a calculator using ALA. Apart from being a cool example of the use of ALA, the calculator itself is cool. This calculator is in Github, fully implemented and working, so you can clone it or download it, run it, and look at any of the details of how it, or any of the domain abstractions work here: https://github.com/johnspray74/ReactiveCalculator[https://github.com/johnspray74/ReactiveCalculator]

This calculator was developed for a workshop on ALA. It was done in a bit of hurry, so here we describe the calculator not so much in terms of how it works, but in terms of the story of its development over several days.

When I was first asked to do a workshop on ALA, I needed to think of a suitable practical project. It was suggested to build a calculator. Ok, I thought, if ALA is as powerful as I am saying it is, we should be able to accomplish that in a two hour workshop. Of course I had in mind to have the domain abstraction done beforehand. Writing these domain abstractions took two Saturday mornings. In the workshop, we would just do the application layer to produce an actual calculator (which I would have pre-reheased).

When we think of a calculator application, we usually imagine a simulated version of a calculator with a one line display and a matrix of keys that include the 10 digits together with +, - * and / keys. We certainly could have built that calculator (primarily using a state machine programming paradigm). But that problem has already been solved by Miro Samek. Besides, such a calculator would be boring and not useful.

I once had a love affair with HP calculators, and have owned many top end models at one time or another. But as their displays got to larger, I became more and more disappointed with them. The HP prime was the pinnacle of this disappointment. What a waste of space, and an opportunity to build a calculator that works half like a calculator and half like a spreadsheet. 

So I drew a sketch of what I thought the HP Prime should have been. Here is the actual sketch I drew: 

image::CalculatorRequirements.png[Calculator3.png, 900, title="Calculator requirements"]

The calculator shows the algebraic expression (formula) you used to get the result so you can check it. You can edit the formula. You can label your formulas. You can use the result of one formula in the formula of another row, using those labels. When you change anything, everything is updated. These are all the things I wanted in a handheld calculator, but with the real reliable keyboard of an HP. Well I wasn't going to get the portability of a calculator, but I could get the rest of that functionality. Ideally you would also be able to enter the formula in RPN, and it would display as textbook or algebraic, as you prefer. But these last two features I have not implemented yet.

Here is a screenshot of the working calculator as it was two days later:

image::CalculatorScreenshot.png[CalculatorScreenshot.png, 900, title="Calculator screenshot"]


The first step in the design of the calculator was to express the requirements (UI plus behaviours). Here is the actual first sketch of the design, done in the first hour. 
...
Each Horizontal has 3 instances of TextBox and one Text. The TextBoxs allow you to enter a string. The abstraction, Text, can only display a string. I see at this point, I didn't put in the TextBox for the description requirement. 

===== How the calculator works

We can ignore the use of labels for a moment and just assume a formula containing constants is typed into the first row TextBox. Let's say we type in "2+1". The string "2+1" goes on the data flow wire to the StringFormat instance on its port 0. The StringFormat is configured with the format string "({1}=>{0})". StringFormat uses this format string in the same way as an interpolated string in C#. The {0} is substituted with the string coming in on port 0. The {1} is substituted with the string coming in on port 1. Since we have no string on port 1, the output from the StringFormat will be "()=>2+1". This is simply a lambda expression with no parameters. This string is fed into the instance of Formula. Formula is an abstraction that knows how to evaluate a lambda expression. Actually it will accept just a formula (such as "2+1" as well). We can ignore the other input of Formula for the moment. Formula will evaluate "()=>2+1" and produce the number 3 on its output. This output is a data-flow of type double. This number is fed to an instance of NumberToString, and from there it goes to the instance of Text that knows how to display a string.

Now let's follow the use of labels in the calculator. Let's put the label "a" in the TextBox for the label in the first row of the calculator. That label, "a", is fed to the Concat instance. Concat's input port can have any number of string data-flows wired into it. In this diagram it has only two. What Concat does is concatenate all its inputs on that port, and can add a separator. In this case the separator is configured to be a comma. The output of the Concat will be "a,". If you were to label the second row "b", the output of the Concat would be "a,b". The concatenated list of labels is fed into port 1 of both StringFormat instances.

Now let's put the formula "a*3" into the 2nd row of the calculator. The output of the StringFormat for that row will now be "(a,)=>a*3". That lambda expression will be fed to the Formula instance. Now the Formula instance, when it sees parameters in the lambda expression, goes and gets values from its other input port. This input port can have many inputs. The first parameter in the lambda expression will use the first input. The second and subsequent parameters in the lambda expression would use the corresponding second and subsequent inputs.

So the Formula will use the first input to get a value for "a". That first input is wired to the result output of the formula in the top row. Then it will go ahead and evaluate "a*3". The output will appear on the corresponding Text in the 2nd row.

So that's all there is to understanding how the calculator works. At this point it takes a leap of faith that the abstractions will all do their jobs, and that the two programming paradigms used, the UI layout and the data-flow, will all work. Not withstanding that, all the information required in the design of this specific calculator is there.
...
I see I forgot to draw Concat. I had to write that one. Here it is from the Xmind templates diagram.

image::CalculatorStringConcatTemplate.png[CalculatorStringConcatTemplate.png, 400, title="StringConcat template"]

The two ports are both drawn on the right hand side unfortunately - a limitation of using Xmind as the drawing tool.  


It was trivial to implement StringConcat. Here is its code in full.
...
. We put a property "InstanceName" in every domain abstraction. It's because ALA achieves high levels of reuse. So you are likely to end up with multiple instances of an sbatrction all over your program. If you name the instances, it make debugging a lot easier because you see it in the debugger and know which instance you are in.

. All the IO for the abstraction is in a section called "Ports". Usually an abstraction would have some ports that are implemented as private fields, and some that are an implemented interface. It just so happens that StringConcat has no implemented interfaces ports.
...
The Formula abstraction has to parse the formula string and then evaluate it. I used Roslyn to do this in a few lines of code. These lines of code took me a few hours to figure out: 
...
Here is the diagram as entered into Xmind. You can click on the image to get a bigger version.
...
Entering the hand-drawn version of the diagram is a simple matter of copying and pasting the Xmind templates for the abstarctions to the right place in the diagram. This conencts most of the needed wiring from port to port. Xmind supports tree structured diagrams, so any cross tree wiring was done by using the red lines, which are quick to put in with cntrl L.

The Xmind version of the diagram is pretty much identical to the hand drawn version. The colored boxes are instances of abstractions. All the other nodes attached around the those colored boxes are the ports. The < and > signs in the ports are significant, and tell the automatic code generator which way to wire the instances. The asterisks are also significant, and tell the code generator that many wires can be wired to the one port.

The diagram done, it's time to generate the code. A tool called XmindParser does this. It can be downloaded from the Github project main page. Here is the tool in use to generate a calculator from the diagram with two rows which was shown above.
...
I usually put the markers inside a method so that all the instantiated objects are private to the function.

Here is the code again with three lines of typical generated code shown in each section.
...
(Any objects that you wish to access elsewhere in Application.cs, you can instantiate them yourself with normal code. Then in the Xmind diagram, put an @ sign on the front of the Abstraction name, and this will tell the code generation tool not to output the code for instantiating the object.)
...
There were two major changes though. One was to put the repeated wiring for a calculator row inside its own abstraction. This abstraction is called "CalculatorRow". This abstraction is less abstract than the Domain abstractions it uses, but more abstract than the application, which will use it multiple times. So it goes into its own layer between the two. This new layer is called "Requirements Abstractions". We don't make new layers lightly, but we had had the experience in a larger project that the diagram got too large. We needed to factor out some abstractions. These abstractions needed a new layer between the Application layer and the Domain abstractions layer. So I was reasonably happy to make use of this new layer in the calculator to reduce now qite large diagram.
...
Because this test calculator has only two rows we can directly compare it with the 2-row calculator above and see how the two rows are now represented by two instances of this new CalculatorRow abstraction. However we are still not there to building a calculator with 10 rows. If we were to instantiate CalculatorRow 10 times, it would need 100 wirings in Xmind to connect all ten results to every row. Having thoughtthe 36 wirings of the 6 row calculator was nuts, there was no way I was going to do 100 of them by hand.

So the next step was to invent a domain abstraction called Multiple.

What multiple does is you instantiate it in your application and configure it with the number you want. In this case N:10. Multiple has a port called factory which uses a new interface called IFactory. Then what you do is add a small Factory class inside any abstraction that you want multiple instances of. In this case we want multiple instances of CalculatorRow, so I added a small class to that abstraction called CalculatorRowFactory. CalculatorRowFactory implements IFactory (which resides in the Programming Paradigms layer). Multiple can now create many instances of the abstraction that is wired to it. But these instances now have to be wired into the rest of the wiring in teh application diagram as needed. To accomplish that, multiple is configured with two methods by the application. These methods, which are part of the application, know how to wire the new instances into the rest of the wiring.
...
This calculator has a Button added to the UI labelled "Add row". I wanted the Button to be at the bottom, so I added another instance of Vertical to achieve that where the new rows will attach. The output of the Button is another programming paradigm interface called an IEvent. 
...
Again, considering that this inetrface enables a whole programming paradigm, it is deceptively simple, consisting of only a synchronous function call. It is the fact that it allows any two objects to be wired together. In teh CalculatorNRows example, it allows a Button to be Wired to a Multiple. When the Button is pressed, Multiple creates a new row.  

There is one major Programming Paradigm that we have not explained yet. It is the one that allows UI instances to be wired together. The meaning of wiring two UI objects together is that ones contains the other. Here is the IUI interface that implements this programming paradigm.
...
Making Multiple able to add rows to the calculator at runtime took yet another Saturday morning. But it was worth it to solve the challenge of learning how to change the wiring at run-time. But it is pretty cool. The wiring diagram now statically describes how it dynamically changes itself.
...
Abstractions are defined in a separate panel as stand-alone box with ports. Once a new abstraction is  defined, it can be instantiated in the diagram by its abstraction name with auto completion. Boxes represent these instances of abstractions with the ports still lablled around their boundary.
