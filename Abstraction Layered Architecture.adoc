...
For a quick summary of ALA read section 3.1 and then 2.1, then perhaps one of the example projects at the end of each chapter.
...
Architectures generally work by applying constraints. ALA's constraint is that you have this one type of relationship. This fundamental constraint emerges all of ALA's properties and patterns that we will explain at in due course. A few of these these are:
...
. An abstraction's implementation can consist of a group of artefacts: local variables, enums, delagate types, interfaces, methods, classes, etc. These artefacts have high cohesion with one another. They are all considered the code for one abstraction. They interconnect with each other, unconstrained. There are no dependency rules among any of the artefacts inside an abstraction. They are all considered part of the same code. The only constraint ALA makes is that their total must be small - of the order of 100-500 lines of code. This applies to all abstractions, including the one that is the application itself in the top layer. Later we will discuss the forces that drive this optimal size.

. Abstractions are the units of code in ALA. They are the only type of encapsulation that works for our brains. Programming languages don't support any concept that corresponds with this idea of an abstraction. So we need to improvise to create the logical grouping, even it can't be an encapsulation enforced by the compiler. (Note that namespaces are not encapsulations). I usually use a source file to represent an abstraction.

. If the code inside an abstraction becomes too large, factor out an abstraction. The factored out abstraction must be more abstract. It can know nothing of the purpose of the abstraction it came from, so it is not just a specific component. It goes in a lower layer, where it can be reused. When the application itself gets too large, we look for abstractions that allow us to make our 'features' separate. For example, if we are building a weather station, we can make an abstraction out of the instance of the display area in the UI. The different features, thermometer, anonomter etc, can then ask this instance for where they should display themselves.

. ALA has no concept of hiercharchies or nesting. Abstractions cannot be contained by other abstractions. They are never nested. The reason they are never nested is simple. An abstraction being used must be more abstract than the abstraction using it. Therefore it must be outside the abstraction using it so it can be reused elsewhere. ALA uses abstraction layers instead of encapsulation hierarchies. In this respect abstractions differ from components in that components can be a specific part of another component. In other words components can be completely contained by the component that uses it.

. Despite the apparent simplicity of the "Standard deviation uses squareroot" example, the constraint of only being allowed to use knowledge dependencies is not trivial to do all of the time. But once the emergent programming paradigms and patterns are practised, it becomes relatively easy. More on them in later chapters.

. There is one aspect of ALA that is hard to master - the invention of appropriate abstractions is relatively difficult. The reason why the "standard deviation uses squareroot" example seems easy is that the squareroot abstraction was already invented for us, and we know it so well. In ALA you will need to invent your own domain level abstractions. In other engineering disciplines, new abstractions come along only every few years, or hundreds of years sometimes. In software engineering, we have to do it every day in the first two weeks of a project, and probably every iteration after that for a few iterations. But all whom I have taught how to do this have found it worth the effort, and all get much better at doing it. Working in the resulting code becomes an joy. 

. The effect of change to any one part of the program, whether in higher layers or lower ones, has greatly reduced ripple effects when compared with conventionally written programs. A ripple generally stops at an abstraction. And since all relationships are just uses of abstractions, ripples usually stay inside a single abstraction. Since abstractions are just concepts they are relatively stable. The quality of the abstractions is important however. The most common change that ripples out is to make an abstraction more abstract to increase its versatility for greater reuse. This can usually be accomplished without breaking the existing abstraction interface (by adding optional parameters or properties).

. We know that ALA requires you to break up your entire application only by factoring out abstractions. So what does the application that's left in the top layer look like when this is done to the maximum extent? Well if anything abstract has been removed, what remains must be details specific only to this application. Essentially these details equate with the requirements. The application becomes just a formal expression of the requirements. There will be some information there that wasn't perhaps explicitly stated in the requirements, but they were requirements none the less. For example, it may not have been stated that a sensor is connected to port 2. Perhaps unusually, an ALA application will have this detail in the very top layer, or at least in a one feature in the top layer (as a configuration parameter to an instance of some abstraction). Or it may not be stated in the requirement that a displayed number should not change its value frequently - it should be smoothed. So the application will end up with a filter wired into the data-flow. It is an emergent property of ALA - the application itself ends up being just a succinct, formal representation of the requirements, which also happens to be executable.
...
Many abstractions are trivial to implement becasue they are zero coupled with anything. They are like tiny stand-alone programs. Here is the full code for StrigConcat.
...
. We put a property "InstanceName" in every domain abstraction. It's not required, but the reason is because abstraction get reused. So you are likely to end up with multiple instances of an abstraction all over your program. If you name the instances, it make debugging a lot easier because you can see it in the debugger and know which instance you are in.
...
At the time of writing I was transitioning from using Xmind to a new diagramming tool for ALA. This tool was in very early beta, so it does not route the cross-wiring around the boxes. However it did generate correct code.
...
Despite having all these great properties that we are taught that all software should be, we are forced to read both functions to understand the code. They are therefore highly coupled. Understanding the code involves understanding the total code in the two functions.  
...
Suddenly understandability is absolutely transformed! Suddenly the coupling between the code inside the two functions is zero. The standard deviation function need only understand the concept of the abstraction, squareroot, not the code that implements it. And clearly, in this case because squareroot is  even more abstract, its code is zero coupled with that inside the standard deviation function.

The complicated code inside SQRT no longer matters. It is completely isolated by the abstraction. If your brain already knows the SQRT abstraction (I had to choose one that I knew you already knew), there is no need to follow the indirection. The reader of the code inside standard deviation continues reading with the next line of code after the SQRT invocation as if it is just like any other line of code in their language. That's what abstraction does.

All those other properties we listed above made no difference. That was because they all describe compile-time properties. They are still needed to make an abstraction, but they are insufficient. The abstraction property was missing. At least for the meantime, abstraction remains a subjective and cerebral activity software engineers must do for themselves. No compiler or tool is going to check it for you.

With this new understanding, we will now define the word dependency to be compile-time relationships, and coupling to be the design-time or understanding-time relationships. One is what the compiler sees, the other is what out brain sees.  

There are two very common situations that may prevent one or other functions being an abstraction:

. If xyz789 is just a source or destination for messages, then abc123 cannot be an abstraction because it cannot be reused without dragging xyz789 with it. In this case xyz789 must be passed into abc123 by something else above both of them. This can be passing in a function, passing in an object (dependency injection), or other mechamism such as the WireTo operator that we will use a lot. 

. If xyz789 provides a specific part of abc123, then xyz789 is more specific than abc123. It needs to be more abstract, not more specific. We are taught to divide and conquer or to separate out the responsibilities. If we do this arbitrarily, we will end up with specific pieces of a part, such as UI and business logic, which are highly coupled. We need to work hard to find abstractions, potentially reusable artefacts, to pull out instead. Then configure those abstractions by passing the details from the more specific abc123 into them.  

ALA's starting premise is to restrict to only having dependencies on abstractions (that are more abstract than we are). This results in zero coupling throughout the entire program. There are other benefits too: these dependencies are now on stable artefacts, as stable as the concept of the abstractions. And, of course, code reuse increases markedly.  
