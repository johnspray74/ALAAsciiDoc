...
Before getting inside the mechanisms of how ALA works, let's explain what problems ALA solves.


=== The Big ball of mud

ALA is a high level strategy to structure code. It tells you how to organise your code so it doesn't degenerate, little by little, into a big ball of mad during it's life cycle. Architectural patterns or styles, such as Layers, Event-Driven, MVC, UML Models, Dependency Injection, Functional Programming and Object Oriented Design are insufficient to prevent a big ball of mud, as are software principles such as Single Responsibility, Liskov Substitution, Interface Segregation, and Dependency Inversion.  What is needed is a higher order organisational strategy. That strategy makes it clear where, how and why to use all these other things. 
...
ALA code is readable, not because of style, convention, comments or documentation, but because any one piece of code appears to you as a separate little program. ALA has zero run-time dependencies, yet does not suffer from the usual problems of indirection. 
--


==== Read this if you have an existing ALA application and you need to start reading the code. 



You will find three folders called:
...
There should also be a readme file that points to this website.  

The starting point is the Application folder. There you should find a diagram that fully describes the requirements of a specific application, and is executable. It should be quite easy to read the diagram as it only describes the requirements and does not involve itself with any implementation. It is declarative, not imperative. There should be a source file that was generated directly from the diagram. Looking at this source file may help clarify questions about how the diagram manages to actually execute.

There are dependencies from the diagram in the Application folder on the abstractions in the DomainaAbstractions folder. The abstractions in the DomainAbstractions folder have no dependencies among themselves, and definitely know nothing about the application. That is why in ALA they are called abstractions rather than classes. Their abstraction level is more general than the application, and aligns with the domain. 

There are dependencies from the code in the abstractions in the DomainAbstractions folder on the abstract interfaces in the ProgrammingParadigms folder. In the ProgrammingParadigms folder. You may also find some documentation about the programming paradigms that these Interfaces make possible. The abstraction level of the ProgrammingParadigms folder is more general again - the paradigms should be useful in a range of domains.  



=== Complexity

There is a meme in the software industry that the complexity of software must be some function of its size. This need not be so. With proper use of abstraction it is possible to have complexity that is constant regardless of program size. ALA makes use of this.
...
If ALA is done well, the maintainability characteristic over time should qualitatively follow the green curve in the graph below. The payback for the effort to learn ALA and design an ALA applciation is enormous. If a project will has a toal life of six months or less (will never be maintained) it may not be worth doing ALA.
...
As has been found in other techniques such such as Domain Specific Languages, Domain Driven Design, Model Driven Software Development and Language Oriented Programming, ALA is highly domain oriented. It has a methodology or process that starts with describing requirements, and inventing needed domain abstractions as you go to easily describe those requirements.   


=== The software engineer's trap

Typical bright young engineers come out of university knowing C++ or Java (or other C*, low-level, imperative, language that mimics the silicon), and are confident that, because the language is Turing-complete, if they string together enough statements, they can accomplish anything. At first they can. Agile methods only require them to deliver an increment of functionality. There hardly seems a need for a software architect to be involved. And besides, we are told that any design be emerged through incremental refactoring.

image::Cynefin.jpg[,500, title="Code can quickly get complex", float="left"]

As the program gets larger, things are getting a little more complicated, but their brains still handle the complications, so they continue. One day the code suddenly 'transitions'. It transitions from the complicated quadrant into the complex quadrant. And now it is trapped there. It is too complex for the in-the-large refactoring that would be required to make it transition back. This pattern happens over and over again in almost all software. The incremental effort to maintain starts to eat away and eventually outweigh the incremental increase in value. This negative return causes the codebase itself to eventually have a negative value, and no longer be an asset to the business. 

When a new bright young engineer who knows C* arrives, he looks at the legacy codebase and is convinced (mostly by Dunning-Kruger bias) that he can do it right. And the cycle starts repeats. This is the CRAP cycle (Create, Repair, Abandon, rePlace). ALA is the only method I know that can prevent the CRAP cycle.

=== My story

From early on in my career, I experienced the CRAP cycle many times. Each time I wanted to find a way to not fall into it. I would read and use all the architectural styles and principles I could find. I would find things like 'loose coupling', and I remember asking myself, yes but how does one accomplish that?, and still fail. I started searching for a pre-worked, generally applicable, 'template architecture' that would tell me what the organisation of the code should look like. I searched for such a thing many times. I never found one. 

Forty years worth of mistakes later, I finally have that template. The turning point was when I noticed two (accidental) successes in parts of two projects. These successes were only noticed years later, 15 years in one case and 5 years in the other. They had each undergone considerable maintenance during that time. But their simplicity had never degraded and their maintenance had always been straightforward. It was like being at a rubbish dump and noticing two pieces of metal that had never rusted. You wonder over to the metal and ask, what is going on here?
...
Analysing the common properties of those two code bases, which no other pieces of our software had, caused the insight of ALA.

As I wrote it down, more and more pieces seemed to fit into place. How it worked made more and more sense. Subsequently, I ran some experiments to see if the quality attributes could be predictably reproduced. These experiments, which have worked spectacularly well so far, are discussed later in this article.
...
Mastering all these topics takes time. Even if you can, juggling them all and being able to use the right ones at the right time, while also mastering technologies and tools, while all the time doing agile sprints is a big ask. In today's agile world, very short term working code is what the team is judged on, especially by project managers or product owners who have no direct interest in architecture. They often don't want to know about the Definition of Done or be involved in the management of (the rather negative sounding term) "technical debt". The overwhelming nature of the knowledge base combine with the rewards of quick 'working' code usually means that software architecture is neglected. 
...
Consequently, ALA contains no truly novel ideas. Instead it is a recipe based on the aforementioned overwhelming list of software engineering topics. Some ingredients are added in much larger quantity than you might expect (such as abstraction). Some are relatively neutral. The biggest surprise for me were the well-established memes in software engineering that ALA says are wrong. We will discuss these one at a time in subsequent passes. Like any good recipe, the ingredients work together to form a whole that is greater than the sum of parts. That whole makes improvements to quality significantly greater than the incremental effects of the topics used independently. It continues to surprise me just how effective it is. 
...
In an Agile world, where every stage of software development, including architecture, emerges from iterative sprints, this wisdom has been lost. ALA restores that wisdom to software development, and gives the software architect the exact process to follow for that piece of up-front design in sprint zero. No more than one sprint is required to do the architecture, regardless of the size of the project.

I believe the result of those first few strokes of the design would never be arrived at by an agile process. Furthermore, once this architecture is done, the Agile process works significantly better thereafter. And my experience over several projects so far is that the architecture does not need to change as the development proceeds. 


=== Example project - Functional composition

In this example, we use 'functional composition' because it is the easiest programming paradigm to use ALA. However, keep in mind that functional composition is not a suitable programming paradigm for most programs. (It only suits when a problem requires dedicated CPU to process a job as fast as it can in computer time, nothing else needs doing while this is happening, and it doesn't have to wait for anything while it is being done. Nevertheless, this is often the case for very small parts of programs, so it's still worth applying ALA to functional composition at this micro scale.
...
For our purpose here, an abstraction means that our brain can easily learn (by reading the function name or a comment) and retain what a function essentially does. It means that when other programmers are reading your code where a function is called, they never have to 'follow the indirection', they can stay with the code unit they are in, they don't have to go and find the function to read code there as well. It should be one simple thing like filter, or get an ADC reading. The name of the function would not be generic ProcessData, or CalculateResult. It would not be the name of the event that caused it to be executed like PulseComplete. They separate a concern but  without abstraction. 

* Abstraction level increases as you go down the levels

The abstraction level should move away from the specifics of the application at the top level, toward more ubiquitous, more reusable, and more stable functions. The 2nd level should have the abstraction level of the domain. The 3rd level should have the abstraction level of the classes of programming problems being solved, such as using third party libraries or what's often referred to as middleware, or writing their equivalents ourselves. For completeness, a 4th level would be your programming language library. No where in these levels is the underlying hardware, nor data. Hardware and data are not abstractions. Later we will see where they go, but for now forget all preconceived notions of layers being built on top of runtime essentials such as hardware or data.  

*  Keep function call tree depth shallow

Often only two levels will be used. The top level corresponds to the detail level of a specific application. The second level corresponds to the abstraction level of the domain. A process of factoring out common code will lead to too many levels and non-abstractions.

* Keep depth levels at discrete levels of abstraction

A function that doesn't clearly belong at one of those abstraction levels may be straddling two levels. This causes all code to have knowledge of requirements. Refactor the function so that application specifc details go to the level above, and an abstract, reusable in the domain function goes below it.
...
This first perspective is the most difficult to understand because it is the most abstract perspective we will use. I introduce it first because it is the one that most differentiates ALA, and gives ALA it its name and structure. 
The diagram shows the abstraction layers:

image::Layers.png[Layers diagram, title="The four ALA layers", width=75%]

The first problem in understanding abstraction layers is understanding what abstraction means. Unfortunately the software industry has misused the word to the point where we get things upside down. This comes about because it sees hardware or data at the bottom, and since hardware and data are 'concrete', it must be the least abstract and so as we build things on top that hardware or data, we must get more abstract. This is completely wrong. A later section goes into depth on what 'abstract' means. For now, just suspend everything you think you know about abstraction, and everything you know about layers. In ALA, neither the hardware nor the data is at the bottom.

Because this perspective doesn't really connect with anything you already know, we will just list three key takeaway points that will gradually become clearer as you get used to them in later parts.
...
This perspective puts the focus on your input information - the requirements. ALA is a methodology that directly describes requirements. And, having described those requirements, it is executable. Instead of having requirements capture and software as two documents with two sources of truth, ALA combines them into a single document and a single source of truth. BDD (Behavioural Driven Design) does something similar, but only achieves it for requirements and their tests. ALA goes one step further and makes the expressed requirements also the executable source code.

===== Executable Architecture

As well as being a description of the requirements, and an executable, the document is also the architecture or the design. (I do not make a distinction between architecture and design.) ALA isn't just an executable requirements description, it is executable architecture.
...
Notice the word 'their', which I have italicised to emphasis that the relations are inferred to be between the said elements. It implies that the elements know something about each other. It implies they collaborate. This is a really bad meme. 
...
An architecture based on "elements and their relations" generally turns out something like this:

image::Slide8.jpg[Elements and their relations, title="Elements and their relations", align="center"]

The figure shows five modules (or classes) and their relations (as interactions). Study almost any piece of software, and this is what you will find.

The structure generally can be viewed as 'clumping'. Like galaxies, certain areas have higher cohesion, and so go in boxes. other areas are loosely coupled, and so in lines. The difference is only one of degree.
...
The word 'modules' has been changed to the word 'abstractions'. All the dependencies are gone. And with them all their problems, and all their management. The implicit coupling that we talked about earlier is also gone. It not longer has a 'clumping' structure. loose coupling is replaced with zero coupling.
...
Consider two well established software abstractions - the 'if statement' and the 'assignment statement'. You would have often composed together two instances of these to make a specific user story. The abstractions 'if statement' and 'assignment statement' don't have any knowledge of each other. The interaction between the instances only happens inside your module, the more specific one that knows about the user story. 

Now imagine that you didn't have the if statement and the assignment statement abstractions when you started. So you started writing an if statement module. Then, from inside that module, you connected it to your assignment statement module. And the assignment statement module, you just made it do what the if statement module needed, just a specific variable. Imagine the opportunity you missed to invent those two abstractions. The two modules you wrote are useless for anything else. And to understand the user story you must understand the insides of both of them, because they collaborate. 

But if you invent the two abstractions, the user story, being just a simple composition of instances of the two, is extraordinarily simple. And the abstractions themselves know nothing of each other, so each of them is simple too. This is the opportunity we are missing, all the time, everywhere in our code, just because of the wording of those memes we listed above.

Imagine the impact this change has. Instead of managing dependencies, which has been the primary focus of software engineering, they are completely gone! We have only one type of dependency left - "an instance of an abstraction". This dependency type turns out to be a good one, a very good one, for it is the one our brains use to understand complicated things such as the world around us. And it turns out that it's just enough dependency to make the system work. Software engineering should not be 'managing dependencies'. It should be 'inventing abstractions'.  
...
The water molecule has "uses instances of" relationship with the two atoms, and the atoms have "uses instances of" relationships with the even more abstract polar bond. Polar bond is an example of what we call an 'abstract interface'.
...
The Frame abstraction is configured with a lambda function to tell it when to stop. These lambda functions are written inside the boxes of the Frame instances. The Frame works like this. It will tell its parent it is complete when both the lambda expression is true and its last child Frame is complete. We have also added a SinglePlay abstraction. 

The idea of the Frame abstraction is that at run-time it will form a composite pattern. As each down-stream child frame completes, a Frame will copy it to start a new one. This will form a tree structure where the "game" will end up with 10 "frames", and each frame will end up with 1, 2 or 3 SinglePlays.
...
This diagram will score 10 frames of ten-pin bowling but does not yet handle strikes and spares. So lets do some 'maintenance' of our application. Because the application so far consists of simple abstractions, which are inherently stable, maintenance should be easy without changing these abstractions.
...
The third variable is the frameNumber. It allows frame objects to know which child they are to their parent -1st child, 2nd child etc. This value is passed to the lambda expression in case it is needed. For example, the lambda expression for a bowling frame needs to know if it is the last frame.  
...
It looks to see if the last child has completed, and if so starts a new child. Then it just passes on the ball score to all the child objects. Any that have completed will ignore it.

The IsComplete method checks both if the last child object is complete and the lambda expression says we are completee:
...
Now let's modify the bowling application to score tennis. If the bowling game hadn't been implemented using ALA, you probably wouldn't contemplate doing this. But ALA has provided us with domain abstractions and a useful programming paradigm for the domain of game scoring. That Frame abstraction looks like it could be useful for the match, which consists of sets, for the set which consists of games, and for the game which consists of SinglePlays.
...
NOTE: The configuration setters and teh WireTo extension method return the object on which the call is made to support the fluent coding style.
...
As discussed in section 2, a common software design approach is stated this way:
...
In ALA, this considered an anti-pattern, and we do the exact opposite. The design method is stated this way:
...
In section 2, we discussed the decomposition/composition difference between these two approaches. Here we review that difference, and look at other differences in the wording of the two approaches. 
...
When you are starting a green fields project, when no abstractions have yet been invented for the domain, this is the methodology to have foremost in your mind.
...
In every creative process outside of software, for example painting or composing music or designing a clock, the process would be much better described as composition of abstractions than decomposition of the final result. Decomposition is just plain backwards.

==== "interacting elements"

The elements resulting from any decomposition process are likely to interact or collaborate to accomplish the purpose of the parent element. The design methodology actually includes the word "interacting". In ALA, it is an anti-pattern to have abstractions interacting. They would no longer be abstractions. 

As discussed in the previous section on "no loose coupling", interactions, which are run-time coupling, are not allowed between design time elements. Instead interactions are represented at design time completely contained inside one abstraction.
