...
image::ComposeClosure.drawio.png[]
...
image::ThreeComposedFunctionsClosureDiagram.drawio.png[]
...
It's exactly the same as what we had before except that we are using Bind instead of Compose. Bind will contain the extra logic to handle -1. The other difference is that the composed functions are now allowed to return -1, which we have done for the third one.
...
The MinusOne is not used outside the C world, and has limited use with only positive integers. The more general solution is the IMaybe<T> monad:

Application layer code
[source,C#]
....
// composition of functions that return IMaybe<T>

IMaybe<int> result = 42.ToMaybe<int>().Bind(function1).Bind(function2).Bind(function3);
....

The Maybe version is similar to the -1 version. However Bind in this case requires an IMaybe and returns an IMaybe. 

IMaybe has two getters, one called HasValue() that returns a bool to find out if a value is there, and the other called Value to get the value out. You would normally use HasValue first and only if it is true use Value. HasValue is analogous to the MoveNext method in the IEnumerator interface, which you also have to call first.

We need to convert 42 to a Maybe so that Bind can be used on it. That's the reason for the ToMaybe extension method.

I have gone back to composing functon1, function2 and function3 to keep the code simpler. The lambda expressions need to return an IMaybe.
...
Func<int> CombinedFunction = 
    42.ToMinusOne().Bind(function1).Bind(function2).Bind(function3);
}
....

The Bind implementation doesn't call the function, it returns another function will do that later: 
...
image::MinusOneDeferredPullMonadDiagram.drawio.png[]
...
We can also do a push version. For the push version we don't have the luck we had in the pull version. Bind requires a real interface. We also need a second interface. The interface that Bind takes and returns must be implemented by a source. But we can't push via such an interface (unless we use C# events). So we have a second interface that is implemented by the destination. The first interfaces function is to facilitate wiring of the second interface in the opposite direction. 

We can't have interfaces on a closure object - a closure can have only one method referenced by a delegate. Therefore we must use an explicit class that implements both interfaces. The interface that Bind uses is called IMinusOneObservable and the interface that is used to push data is called IMinusOneObserver. And yes these two interfaces are analogous to the IObservable and IObserver interfaces in reactive extensions.

Here is the application code that composes the three functons. The return type is IMinusOneObservable:
...
Here is the code for the MinusOne monad, deferred/push version:
...
image::MinusOneDeferredPushMonadDiagram.drawio.png[]
...
Bind creates objects of the class Maybe and chains them using the IMaybe interface.


image::MaybeDeferredPullMonadDiagram.drawio.png[]
...
Now the push version of the deferred IMaybe monad. Here is the applicaation layer code, which in this case returns a IMaybeObservable.
...
As with the deferred push version of the MinusOne monad, we need two interfaces, IMaybeObservabe<T> for Bind to take and return, and IMaybeObserver for doing the actual pushing of data.
...
All IMaybeObservable does is facilitate the wiring of the IMaybeObserver in the opposite direction.

Now we come to the question of what type the functions that you compose should retur.n. For the MinusOne deferred/monad, the functions just returned an integer so that was simple. Conventionally, the functions for the IMaybe monad would return IMaybeObservable, the same interface that Bind takes and returns. So the functions would have the form form: Func<T, IMaybeObservable<U>>. But that would make the functions themselves a little overly complicated. They would have to create an object implementing the IMaybeObservable interface. That interface then has a Subscribe method which you call and pass an IMaybeObserver. Then the object can finally return its value by pushing it via the IMaybeObserver.

It would be much simpler for the functions if we just passed the IMaybeObserver to it when we call it. So the functions would have the form Action<T, IMaybeObserver<U>>. Now the function doesn't need to create an object to return. Instead it can just directly push the result out via the interface. This kind of makes sense because it's a push monad. 

So remember when reading the monad implementation below, the functions that are passed into the Bind fucntion are Actions of the form Action<T, IMaybeObserver<U>>.
...
namespace Monad.MaybeDeferredPush
{
    ...
    class Maybe<T, U> : IMaybeObserver<T>, IMaybeObservabe<U>
    {
        ...
        void IMaybeObservabe<U>.Subscribe(IMaybeObserver<U> observer)
        {
...
The Bind function just creates an object of the Maybe class to do all the work at runtime. It also subscribes that object to the source, which wires it in.

At runtime, the Maybe class will need to call the action, and it needs an object that implements IMaybeObserver to pass to the action so the action can push its result out. That's the purpose of the nested class ActionReceiver. 

The only other things we haven't seen before is to support multiple subscribers. It is normal for push monads to support fan out,in other words many observers can be listening to the data that is pushed out.
